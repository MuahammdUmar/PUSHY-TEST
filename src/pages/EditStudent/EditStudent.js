...
import {
  ...
  GET_TUTOR_BY_ID,
  GET_ALL_COURSES,
  APPROVE_TUTOR_COURSES,
  REJECT_TUTOR_COURSES,
  allTags,
...
const EditStudent = (props) => {
  ...
  const {
    ...
    variables: { id: props && props.studentId },
    skip: props && props.studentId === 0,
    onError: () => {
      props.onHide();
    },
  });
  ...
  const [approveCourses, { loading: loadingApprove, error: errorApprove }] =
    useMutation(APPROVE_TUTOR_COURSES, {
      onError: () => {
      ...
      onCompleted: () => {
        addToast("Tutor courses approved successfully", {
          appearance: "success",
        });
      },
  ...
  const [rejectCourses, { loading: loadingReject, error: errorReject }] =
    useMutation(REJECT_TUTOR_COURSES, {
      onError: () => {
        //setDeleteDialogOpen(false)
      },
      variables: {
      ...
      onCompleted: () => {
        addToast("Tutor courses rejected successfully", {
          appearance: "success",
        });
        setSelectedCourses([]);
      },
  ...
  const [update, { loading, error, data }] = useMutation(UPDATE_USER, {
    ...
    onCompleted: (data) => {
      addToast("Student saved Successfully", { appearance: "success" });
      props.onHide();
  ...
  useEffect(() => {
    if (dataStudent) {
      ...
      if (dataStudent.UsersById.DOB !== null) {
        ...
        setDob(moment(dataStudent.UsersById.DOB).format("YYYY-MM-DD"));
        setDobISO(dataStudent.UsersById.DOB);
      }
      setAddressOne(dataStudent.UsersById.Address1);
      //setAllCourses(dataStudent.UsersById.Courses)
      if (dataStudent.UsersById.Courses !== null) {
        setSelectedCourses(
          dataStudent.UsersById.Courses.map((course) => ({
            value: course.id,
            ...
            {/* <div className='row mb-3'>
                <div className='col-12'>
                  <Select
                    ...
                    options={(allCourses !== null) ? allCourses.map((c) => ({ value: c.id, label: c.CourseName })) : []}
                    isClearable
                    isMulti
                  />
                  <div className="mt-2">
                    <Button variant="primary" onClick={approveTutCours}>Approve</Button>{' '}
                    <Button variant="danger" onClick={rejectTutCours}>Decline</Button>
                  </div>
                </div>
              </div> */}
